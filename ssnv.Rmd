...
```{r mapviewskB,echo = FALSE,	fig.align = "center",	message = FALSE,	warning = FALSE,	out.width='100%',	error=FALSE, fig.cap='Siglt var á kajak upp að bauju við grásleppunet (sem notuð var sem ankeri) á milli Eyrarinnar og Skeljatanga rétt innan við Skagaströnd. Tekin voru myndskeið til að greina tegundir þara og meta vöxt hans.'}
library(XML)
library(OpenStreetMap)
library(lubridate)
library(raster)
...
rass=rgdal::readOGR(dsn="C:/Users/BioPol VS/Documents/Vinnumappa/GIS/Isl_dlin_shape/Isl_dyp_lin_290115.shp",verbose = FALSE)

#búa til hringlaga svæði
library(sampSurf)

Radius = 0.2
midjaX=(bbox(Zodiac)[1]+bbox(Zodiac)[3])/2
midjaY=(bbox(Zodiac)[2]+bbox(Zodiac)[4])/2
hringur = spCircle(radius = Radius, centerPoint=c(x=midjaX,y=midjaY), spID='ekkert') 

proj4string(hringur$spCircle) <- "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
e2 <- sp::spTransform(hringur$spCircle, raster::crs(rass))
