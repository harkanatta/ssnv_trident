...
```{r trident, fig.cap='Tækið sem fékkst fyrir styrkféð frá SSNV kallast [Trident](https://www.openrov.com/) og er framleitt af OpenRov. Hér sést Karin Zech starfsmaður BioPol rýna á skjáinn á fjarstýringunni að tækinu og Guðmundur Björnsson fyrrv. grásleppuveiðimaður var okkur til halds og trausts. ', out.width='35%', fig.align='center', echo=FALSE, fig.show = 'hold'}
...
```{r mapv, echo=FALSE, error=F, message=FALSE, warning=FALSE, comment=NA, fig.cap='Siglt var á kajak upp að bauju við grásleppunet (sem notuð var sem ankeri) á milli Eyrarinnar og Skeljatanga rétt innan við Skagaströnd. Tekin voru myndskeið til að greina tegundir þara og meta vöxt hans.',out.width='100%'}
library(XML)
...
#class(spdf_geo)
sk1 <- rgdal::readOGR(dsn="skjol/skogar/sk1/sk1.shp")
...
## Forkönnun við Stekkjavík og Sölvabakka 2019

Farið var á Zodiac inn að Sölvabakka föstudaginn 21. júní. Báturinn er í eigu björgunarsveitrinnar Stráka á Skagaströnd, Einn maður um borð og var Trident kafbáturinn meðferðis. Kafbáturinn var settur niður á þremur stöðum á milli Stekkjavíkur og Sölvabakka

```{r mapviewsk2, echo=FALSE, error=F, message=FALSE, warning=FALSE, comment=NA, fig.cap='Siglt var á kajak upp að bauju við grásleppunet (sem notuð var sem ankeri) á milli Eyrarinnar og Skeljatanga rétt innan við Skagaströnd. Tekin voru myndskeið til að greina tegundir þara og meta vöxt hans.',out.width='100%'}
library(XML)
library(OpenStreetMap)
library(lubridate)
library(raster)
...
#class(spdf_geo)
#sk1 <- rgdal::readOGR(dsn="skjol/skogar/sk1/sk1.shp")
rass=rgdal::readOGR(dsn="C:/Users/BioPol VS/Documents/Vinnumappa/GIS/Isl_dlin_shape/Isl_dyp_lin_290115.shp")
#búa til kassalaga svæði
library(sp)
e <- as(raster::extent(-21.51, -20,65.38, 66.1), "SpatialPolygons")
#e <- as(raster::extent(bbox(Zodiac)[1]+bbox(Zodiac)[1]*0.01,bbox(Zodiac)[3]+bbox(Zodiac)[3]*(-0.01),bbox(Zodiac)[2]-bbox(Zodiac)[2]*0.001,bbox(Zodiac)[4]+bbox(Zodiac)[4]*0.001), "SpatialPolygons")
proj4string(e) <- "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
e2 <- sp::spTransform(e, raster::crs(rass))
clp1 <- raster::crop(rass,e2)
library(RColorBrewer)
pal <- colorRampPalette(brewer.pal(length(unique(clp1@data$MinZ)), "Blues"))
mapview(Zodiac,color="red",col.regions="grey",map.types="Esri.WorldImagery")+mapview(clp1,zcol="MinZ",color = pal)
...
```{r mapviewsk3, echo=FALSE, error=F, message=FALSE, warning=FALSE, comment=NA, fig.cap='Siglt var á kajak upp að bauju við grásleppunet (sem notuð var sem ankeri) á milli Eyrarinnar og Skeljatanga rétt innan við Skagaströnd. Tekin voru myndskeið til að greina tegundir þara og meta vöxt hans.',out.width='100%'}
library(XML)
library(OpenStreetMap)
library(lubridate)
library(raster)
...
```{r mapviewsk4, echo=FALSE, error=F, message=FALSE, warning=FALSE, comment=NA, fig.cap='Siglt var á kajak upp að bauju við grásleppunet (sem notuð var sem ankeri) á milli Eyrarinnar og Skeljatanga rétt innan við Skagaströnd. Tekin voru myndskeið til að greina tegundir þara og meta vöxt hans.',out.width='100%'}
library(XML)
library(OpenStreetMap)
library(lubridate)
library(raster)
